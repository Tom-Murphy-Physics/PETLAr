...
void plot_attach()
{
  ...
  for (double lambda = 0.2; lambda <=3.0; lambda = lambda + 0.4)
  {
	...
	cout<<ys*exp(-lambda*(xs*25.4-ysd)/1000)<<endl;
      
	gr->SetPoint(ns, xs, ys*Recombination_factor*exp(-lambda*(xs*25.4-ysd)/1000));
	gr->SetPointError(ns, exs, eys*Recombination_factor*exp(-lambda*(xs*25.4-ysd)/1000));
