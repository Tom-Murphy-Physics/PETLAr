...
void plot_chargepp()
{
  ...
  for (double lambda = 0.2; lambda <=3.0; lambda = lambda + 0.4)
  {
	...
	gr->SetPoint(ns, ixs, iys/eys*Recombination_factor*exp(-lambda*(288.036-ysd)/1000));
	gr->SetPointError(ns, iexs,sqrt(ieys*ieys + eeys*eeys)*Recombination_factor*exp(-lambda*(288.036-ysd)/1000));
  ...
  mg->Draw("AP");
  mg->SetTitle("Charge at Pixels per Electron Scattering;Radius of Detector (in);Number of Ionization Electrons at Pixels");
