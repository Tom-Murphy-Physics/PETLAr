...
void plot_ratio_attach()
{
  ...
  for (double lambda = 0.2; lambda <=3.0; lambda = lambda + 0.4)
  {
	...
	gr->SetPoint(ns, xs, ys*Recombination_factor*exp(-lambda*(150-ysd)/1000)/ys);
	gr->SetPointError(ns, exs, eys*Recombination_factor*exp(-lambda*(150-ysd)/1000)/ys);
  ...
  mg->Draw("AP");
  mg->SetTitle("Percent of Charge at Pixels per Event;Radius of PMT (mm);Percent of Ionization Electrons at Pixels");
