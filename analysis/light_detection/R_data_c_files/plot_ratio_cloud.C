...
void plot_ratio_cloud()
{
  ...
  for (double lambda = 0.2; lambda <=3.0; lambda = lambda + 0.4)
  {
    ...
    fstream file;
    file.open("../R_data_txt/cloud_data.txt", ios::in);
	...
	if (ys != 0)
	  {
	    gr->SetPoint(ns, xs, (ys + 4*sqrt((2*Dt*(150-ysd))/V))/ys);
	    gr->SetPointError(ns, exs, (eys+4*sqrt((2*Dt*(eysd))/V))/ys);
	  }
	if(file.eof()) break;
      }
  ...
  mg->Draw("AP");
  mg->SetTitle("Pixel Length to MC Track Length (96CL);Radius of Detector (mm);Ratio of Legth at Pixels to True Length");
   mg->GetYaxis()->SetRangeUser(0,50);
  c1->BuildLegend();
  c1->SaveAs("Cloud_Size_ratio_plt.png");
}
