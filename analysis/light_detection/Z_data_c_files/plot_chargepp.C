...
void plot_chargepp()
{
  ...
  for (double lambda = 0.2; lambda <=3.0; lambda = lambda + 0.4)
  {
    ...
    fstream file2;
    file2.open("../Z_data_txt/enum_data.txt", ios::in);
	...
	gr->SetPoint(ns, ixs, iys/eys*Recombination_factor*exp(-lambda*(xsd-ysd)/1000));
	gr->SetPointError(ns, iexs,sqrt(ieys*ieys + eeys*eeys)*Recombination_factor*exp(-lambda*(xsd-ysd)/1000));
  ...
  mg->Draw("AP");
  mg->SetTitle("Charge at Pixels per Electron Scattering;Drift Length (mm);Number of Ionization Electrons at Pixels");
