...
void plot_attach()
{
  ...
  for (float lambda = 3.0; lambda <=3.0; lambda = lambda + 3)
  {
	...
	gr1->SetPoint(ns1, xs1, ys1*Recombination_factor*exp(-lambda*(288.036-ysd1)/1000));
	gr1->SetPointError(ns1, exs1, eys1*Recombination_factor*exp(-lambda*(288.036-ysd1)/1000));
	...
	gr2->SetPoint(ns2, xs2, ys2*Recombination_factor*exp(-lambda*(288.036-ysd2)/1000));
	gr2->SetPointError(ns2, exs2, eys2*Recombination_factor*exp(-lambda*(288.036-ysd2)/1000));
	...
	gr3->SetPoint(ns3, xs3, ys3*Recombination_factor*exp(-lambda*(288.036-ysd3)/1000));
	gr3->SetPointError(ns3, exs3, eys3*Recombination_factor*exp(-lambda*(288.036-ysd3)/1000));	
	if(file1.eof()) break;
	...
	gr4->SetPoint(ns4, xs4, ys4*Recombination_factor*exp(-lambda*(288.036-ysd4)/1000));
	gr4->SetPointError(ns4, exs4, eys4*Recombination_factor*exp(-lambda*(288.036-ysd4)/1000));	
	if(file1.eof()) break;
  ...
  mg->Draw("AP");
  mg->SetTitle("Charge at Pixels per Event;Radius of Detector (in);Number of Ionization Electrons at Pixels");
  mg->GetYaxis()->SetRangeUser(0,1250);
