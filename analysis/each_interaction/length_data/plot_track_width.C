...
void plot_track_width()
{
  ...
  for (double xp = 0.001; xp <=0.005; xp = xp + 0.001)
  {
	...
	//cout<<eysd<<endl;
	double xc = (-3*pow(xp,2)+sqrt(9*pow(xp,4)-12*xp*(pow(xp,3)+3*pow(4*Dt*(xsd*25.4-ysd)/(1000*V),1.5))*(2*noise*sigma/(N0*exp(-lambda*V*(xsd*25.4-ysd)/1000/V))-2*xp/sigma)))/(3*xp);
	cout<<xc<<endl;
	gr->SetPoint(nsd, xsd, 1000*xc);
	gr->SetPointError(nsd, exsd, xc/xp/6*sqrt(36*xp*(2*noise*sqrt(3.141592653589)*sqrt(pow(eysn/N0,2)+pow(4*Dt,2)*pow(2*eysd/(xsd*25.4-ysd),2))-2*xp*Dt*4*eysd/(xsd*25.4-ysd))));
